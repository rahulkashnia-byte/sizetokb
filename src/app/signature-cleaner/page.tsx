...
export default function SignatureCleanerPage() {
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const r = await cleanSignature(file, { minKb, maxKb, threshold });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <div className="text-center">
        ...
        <p className="mt-3 text-[var(--muted)]">High-contrast cleanup, auto-crop, and KB target for form signatures.</p>
        <TrustPills />
      </div>
      <div className="mt-8 rounded-3xl border border-[var(--line)] bg-white p-5 shadow-[var(--card-shadow)] sm:p-7">
        ...
        <label className="mt-3 block text-sm font-semibold">
          ...
          <input type="range" min={100} max={220} value={threshold} onChange={(e) => setThreshold(Number(e.target.value))} className="mt-2 w-full" />
        </label>
