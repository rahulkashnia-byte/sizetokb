...
export function SevaDeskPartnerStrip({
  ...
  const examLink =
    examSlug && examName ? sevadeskForExam(examSlug, examName) : null;
    ...
    <aside className="mx-auto max-w-6xl px-4 pb-8 sm:px-6">
      <div className="rounded-2xl border border-[var(--line)] bg-[var(--wash)] px-5 py-4 sm:px-6 sm:py-5">
        ...
        <p className="mt-1 max-w-2xl text-sm text-[var(--muted)]">
          SevaDesk summarises government job and exam notices with official links — then come back here to hit the photo / signature KB limit.
        </p>
  ...
  return (
    <div className="mt-10 border-t border-white/10 pt-8">
      <h4 className="text-xs font-bold uppercase tracking-wider text-white/45">
        Government jobs &amp; results · {SEVADESK.name}
      </h4>
      <p className="mt-2 max-w-2xl text-sm text-white/65">
        After you resize your photo, check vacancies, last dates, results and admit cards on SevaDesk.
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        {SEVADESK_NAV.map((item) => (
          <a
            key={item.path}
            href={sevadeskPath(item.path)}
            target="_blank"
            rel="noopener noreferrer"
